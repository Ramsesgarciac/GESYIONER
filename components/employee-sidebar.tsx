...
export function EmployeeSidebar({ isOpen, onToggle }: EmployeeSidebarProps) {
  ...
  const isInactivos = pathname === "/inactivos"

  return (
    <aside
      className={`h-screen bg-sidebar transition-all duration-300 ease-in-out flex flex-col ${isOpen ? "w-56" : "w-14"
        }`}
    >
      ...
      <nav className="px-2 space-y-1 mt-4">
        <Link
          href="/"
          className={`flex items-center gap-3 px-3 py-2.5 rounded-md font-medium transition-colors ${!isInactivos
