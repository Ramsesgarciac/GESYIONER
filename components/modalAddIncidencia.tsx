...
import { CreateIncidenciaDto, Incidencia } from "@/types/Incidencias"
import { createIncidencia } from "@/lib/services/IncidenciaService.service"
import { AddJustificanteModal } from "@/components/modalAddJustificante"
...
export function AddIncidenciaModal({ open, onClose, onSuccess, id_empleado }: AddIncidenciaModalProps) {
    ...
    // Estado para el modal de justificante que se abre después
    const [incidenciaCreada, setIncidenciaCreada] = useState<Incidencia | null>(null)
    const [justificanteModalOpen, setJustificanteModalOpen] = useState(false)

    const { tiposIncidencias, loading: loadingTipos } = useTiposIncidencias()

    if (!open && !justificanteModalOpen) return null
    ...
    const handleSubmit = async () => {
        ...
        setSubmitError(null)
        try {
            const nueva = await createIncidencia({
                id_empleado,
            ...
            // Guardar la incidencia creada y abrir modal de justificante
            setIncidenciaCreada(nueva)
            setJustificanteModalOpen(true)
        } catch (err) {
    ...
    const handleJustificanteClose = () => {
        setJustificanteModalOpen(false)
        onSuccess()
        handleClose()
    }

    const descripcionIncidencia = selectedTipo?.nombre
        ? `${selectedTipo.nombre} (${form.fecha_inicio} — ${form.fecha_fin})`
        : undefined

    return (
        <>
            {/* Modal principal de incidencia */}
            {open && !justificanteModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden">
                        ...
                        <div className="px-6 pt-6 pb-4 border-b border-border">
                            <button onClick={handleClose} className="absolute top-4 right-4 p-1.5 rounded-full hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-colors">
                                <X className="w-4 h-4" />
                            </button>
                                            ...
                                            {tiposIncidencias.length === 0 ? (
                                                ...
                                                tiposIncidencias.map(tipo => (
                                                    <button
                                                        ...
                                                        type="button"
                                                        onClick={() => { handleChange("id_tipo_incidencia", tipo.id_tipo_incidencia); setTipoOpen(false) }}
                                                        className={`w-full text-left px-3 py-2.5 text-sm hover:bg-muted/40 transition-colors ${form.id_tipo_incidencia === tipo.id_tipo_incidencia ? "bg-primary/10 text-primary font-medium" : "text-foreground"}`}
                                                    >
                        ...
                        <div className="px-6 pb-6 flex gap-3">
                            ...
                            <Button className="flex-1 bg-primary hover:bg-primary/90 text-primary-foreground" onClick={handleSubmit} disabled={submitting}>
                                {submitting ? <><Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" />Guardando...</> : "Siguiente"}
                            </Button>
                        </div>
                    </div>
                </div>
            ...
            {/* Modal de justificante que se abre después */}
            {incidenciaCreada && (
                <AddJustificanteModal
                    open={justificanteModalOpen}
                    onClose={handleJustificanteClose}
                    onSuccess={handleJustificanteClose}
                    id_incidencia={incidenciaCreada.id_incidencia}
