...
import {
  FileText, Plus, Download, Loader2, CheckCircle2,
  XCircle, User, Hash, Briefcase, MapPin, Tag,
  CreditCard, IdCard, AlertCircle, BadgeCheck, ChevronRight
} from "lucide-react"
...
const incidencias = [
  { tipo: "Permiso no justificado", fechaInicio: "12/09/2016", fechaFin: "14/09/2016" },
...
export default function EmpleadoPage() {
  ...
  const { empleado, loading: loadingEmpleado, error: errorEmpleado } = useEmpleado(empleadoId)
                ...
                {!loadingEmpleado && empleado && (
                  <div className="bg-white rounded-2xl border border-border p-5">
                    ...
                    <div className="divide-y divide-border/40">
                      ...
                      <InfoRow icon={Hash} label="Salario actual" value={empleado.salario_actual ? `$${Number(empleado.salario_actual).toLocaleString("es-MX")}` : "No especificado"} />
                    </div>
                  </div>
                )}
              </div>

              {/* Columna derecha (3/5) — Documentación */}
                    ...
                    <div className="mb-4">
                      ...
                      <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
                        <div
                          ...
                          style={{ width: `${listado.estadisticas.porcentaje_completado}%` }}
                        />
                      </div>
                    </div>
                  )}
                  ...
                  {loadingDocs ? (
                    ...
                    <div className="space-y-2 max-h-[calc(100vh-360px)] overflow-y-auto pr-0.5">
                      {listado.documentos.map((doc) => (
                        <div
                          key={doc.id_tipo_doc}
                          className={`group rounded-xl border p-3.5 transition-all duration-200 ${doc.subido
                            ? "border-primary/20 bg-primary/4 hover:border-primary/30"
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                {doc.obligatorio && !doc.subido && (
                                  <span className="text-[10px] bg-amber-50 text-amber-700 border border-amber-200 px-1.5 py-0.5 rounded-full font-medium shrink-0">
                                    Obligatorio
                                  </span>
                                )}
                              </div>
                              {doc.descripcion && (
                                <p className="text-[11px] text-muted-foreground mt-0.5">{doc.descripcion}</p>
                              )}
                              {doc.subido && doc.documento ? (
                                <p className="text-[11px] text-primary/70 mt-0.5 truncate">
                                  {doc.documento.nombre_archivo} · v{doc.documento.version} · {new Date(doc.documento.fecha_carga).toLocaleDateString("es-MX")}
                                </p>
                              ) : (
                            ...
                            <div className="shrink-0">
                              {doc.subido && doc.documento ? (
                                <button
                                  ...
                                  className="flex items-center gap-1 text-[11px] text-primary hover:text-primary/80 font-medium transition-colors bg-primary/8 hover:bg-primary/15 px-2.5 py-1.5 rounded-lg"
                                >
          ...
          {activeTab === "incidencias" && (
            <div className="space-y-5">
              <div className="flex items-start gap-4">
                ...
                <Button className="bg-btn-blue hover:bg-btn-blue-hover text-white shrink-0">
                  ...
                  <TableBody>
                    {incidencias.map((incidencia, index) => (
                      <TableRow key={index} className="hover:bg-muted/30 border-b border-border/50">
                        <TableCell className="text-center py-3.5 text-sm">{incidencia.tipo}</TableCell>
                        <TableCell className="text-center py-3.5 text-sm text-muted-foreground">{incidencia.fechaInicio}</TableCell>
                        <TableCell className="text-center py-3.5 text-sm text-muted-foreground">{incidencia.fechaFin}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
